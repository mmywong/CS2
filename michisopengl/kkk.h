...
void readfile(int world[max_row][max_col]);
...
void Convert(int world[max_row][max_col]);
...
//converts position in array to position in x,y graph format
void Convert(int world[max_row][max_col])
{
    int x, y; //coordinates of the top left hand corner of each square pixel
    for(int i=0; i<max_row; i++)
        for(int j=0; j<max_col; j++)
        {
            x = -1 + (2/max_col)*j;
...
/* old stuff:
            ...
            if(j = (max_col/2))
                x = 0;
            if(i < (max_row/2) || i > (max_row/2))
                y = i - (max_row/2);
            if(j < (max_col/2) || j > (max_col/2))
                x = j - (max_col/2);
*/
        }
}
